...
class TestAuditRecorder:
    ...
    def test_version_field_populated(self, tmp_path: Path) -> None:
        """Recorded entries carry the package version string."""
        from onefinance.audit._recorder import _APP_VERSION
        ...
        entries = audit_log.query()
        assert entries[0].version == _APP_VERSION

    def test_version_in_to_dict(self) -> None:
        entry = _entry(version="1.2.3")
        d = entry.to_dict()
        assert d["version"] == "1.2.3"

    def test_version_none_in_to_dict(self) -> None:
        entry = _entry()
        d = entry.to_dict()
        assert "version" in d
...
# ---------------------------------------------------------------------------
# Version-based truncation
# ---------------------------------------------------------------------------


class TestTruncateBeforeVersion:
    def test_removes_older_version_entries(self, tmp_path: Path) -> None:
        log = AuditLog(log_path=tmp_path / "audit.jsonl")
        log.record(_entry(version="0.1.13"))
        log.record(_entry(version="0.1.14"))
        log.record(_entry(version="0.1.15"))

        removed = log.truncate_before_version("0.1.15")
        assert removed == 2
        entries = log.query()
        assert len(entries) == 1
        assert entries[0].version == "0.1.15"

    def test_removes_versionless_entries(self, tmp_path: Path) -> None:
        log = AuditLog(log_path=tmp_path / "audit.jsonl")
        # old-style entry without version
        ...
        log.record(_entry(version="0.1.16"))

        removed = log.truncate_before_version("0.1.16")
        assert removed == 1
        entries = log.query()
        assert len(entries) == 1
        assert entries[0].version == "0.1.16"

    def test_keeps_equal_version(self, tmp_path: Path) -> None:
        log = AuditLog(log_path=tmp_path / "audit.jsonl")
        log.record(_entry(version="0.1.15"))
        log.record(_entry(version="0.1.16"))
    ...
    def test_noop_on_empty_log(self, tmp_path: Path) -> None:
        log = AuditLog(log_path=tmp_path / "audit.jsonl")
        assert log.truncate_before_version("0.1.15") == 0

    def test_bad_version_string_raises(self, tmp_path: Path) -> None:
        from packaging.version import InvalidVersion

        log = AuditLog(log_path=tmp_path / "audit.jsonl")
        with pytest.raises(InvalidVersion):
            log.truncate_before_version("not-a-version")
