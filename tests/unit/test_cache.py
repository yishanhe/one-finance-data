...
class TestSerialisation:
    ...
    def test_list_model_round_trip(self):
        ...
        envelope = _serialise_envelope(bars)
...
class TestCacheManagerGetSet:
    ...
    def test_set_and_get_single(self, cache: CacheManager):
        info = _make_info()
    ...
    def test_set_and_get_list(self, cache: CacheManager):
        bars = [_make_bar("AAPL", date(2024, 1, 2)), _make_bar("AAPL", date(2024, 1, 3))]
        cache.set("price_history:xyz789", bars, ttl=3600, tag="price_history")
        result = cache.get("price_history:xyz789")
        assert isinstance(result, list)
        assert len(result) == 2

    def test_expired_entry_returns_none(self, cache: CacheManager):
        info = _make_info()
    ...
    def test_set_empty_list(self, cache: CacheManager):
...
class TestStats:
    ...
    def test_after_inserts(self, cache: CacheManager):
        cache.set("info:a", _make_info(), ttl=3600, tag="info")
        cache.set("info:b", _make_info("MSFT"), ttl=3600, tag="info")
        s = cache.stats()
        assert s["entries"] == 2
...
class TestSmartTTLPriceHistory:
    ...
    def test_includes_today_market_closed(self):
